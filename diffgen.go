...
var (
	all      = flag.Bool("all", false, "include unexported fields")
...
func main() {
	...
	fields := d.ProcessStruct(nil, d.base)
	if len(fields) == 0 {
		log.Fatal("no fields found. need '-all'?")
	}
	c := Comparisons{
	...
	for _, v := range fields {
		if slices.Contains(v, "[slice]") {
			hasSlice = true
		}
		if slices.Contains(v, "[map]") {
			hasMap = true
		}
	...
	case 0:
	case 1:
		imp = fmt.Sprintf("import %q", imports[0])
	default:
		imp = "import (\n"
...
func (d *DiffGen) ProcessStruct(path []string, s *types.Struct) [][]string {
...
loop:
	for i := 0; i < s.NumFields(); i++ {
		...
		case f.Exported():
			// process
		case f.Pkg().Path() != d.PkgPath, !*all:
			continue loop
		}
		nPath := make([]string, 0, len(path)+1)
		nPath = append(nPath, path...)
		res := d.ProcessType(append(nPath, f.Name()), f.Type())
		comparisons = append(comparisons, res...)
	}
	return comparisons
}

func (d *DiffGen) ProcessType(prefix []string, t types.Type) [][]string {
	switch t := t.(type) {
	case *types.Pointer:
		elem := t.Elem()
		items := d.ProcessType(append(prefix, "[pointer]"), elem)
		// methods
	...
	case *types.Named:
		...
		return d.ProcessType(prefix, obj.Type().Underlying())
	case *types.Struct:
		return d.ProcessStruct(prefix, t)
	case *types.Slice:
		elem := t.Elem()
		return d.ProcessType(append(prefix, "[slice]"), elem)
	case *types.Map:
		_, ok := t.Key().Underlying().(*types.Basic)
		if !ok {
			log.Fatal("only basic types for slice supported at the moment")
		}
		return d.ProcessType(append(prefix, "[map]"), t.Elem())
	case *types.Array, *types.Basic, *types.Interface:
		return [][]string{prefix}
